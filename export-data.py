...
QUERIES: dict[str, str] = {
    ...
    # the ~3,200 schools with no marketKey. Enrollment figures come from the
    # dbo.Enrollments view (school x year); dbo.Schools supplies identity /
    # market mapping. Schools_Denormal flagged do-not-use 2026-07-14
    # (Enrollments_Manual was flagged the same day).
    "enrollment_total": """
        SELECT
            s.SchoolKey                                                  AS university_key,
            s.name                                                       AS university_name,
            s.MarketKey                                                  AS market_key,
            e.enrollmentTotal                                            AS total_enrollment,
            e.enrollmentFullTimeUndergraduate
              + e.enrollmentPartTimeUndergraduate                        AS total_undergrad,
            e.enrollmentYear                                             AS academic_year
        FROM dbo.Schools s
        JOIN dbo.Markets m ON m.[Key] = s.MarketKey
        JOIN dbo.Enrollments e ON e.schoolKey = s.SchoolKey
        WHERE e.enrollmentTotal > 0
    """,
    ...
    """,
    # university_info: per-school institutional stats - on-campus beds,
    # tuition, admissions funnel, housing rates, student profile. Drives the
    # University Information tab on the market page. Figures come from the
    # dbo.Enrollments view (school x year; keep the latest year with real
    # enrollment, one row per school); dbo.Schools supplies identity / market
    # mapping. Schools_Denormal flagged do-not-use 2026-07-14. Zeros in
    # rate / income columns mean "not reported" and are treated as missing
    # by the UI.
    "university_info": """
        WITH ranked AS (
            SELECT e.*,
                   ROW_NUMBER() OVER (
                       PARTITION BY e.schoolKey
                       ORDER BY e.enrollmentYear DESC
                   ) AS rn
            FROM dbo.Enrollments e
            WHERE COALESCE(e.enrollmentTotal, 0) > 0
        )
        SELECT
            s.MarketKey                                    AS market_key,
            ...
            e.enrollmentYear                               AS enrollment_year,
            e.enrollmentTotal                              AS enrollment_total,
            e.enrollmentFullTimeUndergraduate              AS enr_ft_undergrad,
            e.enrollmentPartTimeUndergraduate              AS enr_pt_undergrad,
            e.enrollmentFullTimeGraduate                   AS enr_ft_grad,
            e.enrollmentPartTimeGraduate                   AS enr_pt_grad,
            e.enrollmentUndergradInStatePct                AS pct_in_state,
            e.enrollmentUndergradOutOfStatePct             AS pct_out_of_state,
            e.enrollmentUndergradOnCampusPct               AS pct_on_campus,
            e.enrollmentUndergradOffCampusPct              AS pct_off_campus,
            e.bedsOnCampusReported                         AS beds_on_campus_reported,
            e.bedsOnCampusComputed                         AS beds_on_campus_computed,
            e.rateOnCampusRoomYearly                       AS rate_room_yearly,
            e.rateOnCampusBoardYearly                      AS rate_board_yearly,
            e.rateOnCampusRoomMonthlyAvg                   AS rate_room_monthly,
            e.costHousingDelta                             AS cost_housing_delta,
            e.tuitionInState                               AS tuition_in_state,
            e.tuitionOutOfState                            AS tuition_out_of_state,
            e.costPerCreditHourUndergradInState            AS credit_hour_in_state,
            e.costPerCreditHourUndergradOutOfState         AS credit_hour_out_of_state,
            e.appliedFirstTimeFirstYear                    AS applied_first_year,
            e.admittedFirstTimeFirstYear                   AS admitted_first_year,
            e.admittedFirstTimeFirstYearPct                AS admit_rate,
            e.enrolledFirstTimeFirstYear                   AS enrolled_first_year,
            e.appliedTransfer                              AS applied_transfer,
            e.admittedTransfer                             AS admitted_transfer,
            e.admittedTransferPct                          AS transfer_admit_rate,
            e.enrolledTransfer                             AS enrolled_transfer,
            e.studentAgeAvg                                AS student_age_avg,
            e.amountParentIncomeAvg                        AS parent_income_avg,
            e.amountParentIncomeMed                        AS parent_income_med,
            e.hasDormsCoed                                 AS has_dorms_coed,
            e.hasDormsMen                                  AS has_dorms_men,
            e.hasDormsWomen                                AS has_dorms_women,
            e.hasApartmentsMarried                         AS has_apts_married,
            e.hasApartmentsSingle                          AS has_apts_single,
            e.hasHousingDisabled                           AS has_housing_disabled,
            e.hasHousingIntl                               AS has_housing_intl,
            e.hasHousingGreek                              AS has_housing_greek,
            e.hasHousingCoop                               AS has_housing_coop
        FROM dbo.Schools s
        JOIN ranked e ON e.schoolKey = s.SchoolKey AND e.rn = 1
        WHERE s.MarketKey IS NOT NULL
        ORDER BY s.MarketKey, s.name
    """,
    # admissions_history: first-time first-year funnel per school per year
    # from the dbo.Enrollments view (2014+, 2025 included). Replaces the
    # IPEDS ADM Excel repository (build_admissions_repository.py ->
    # load_admissions_history.py), which stopped at 2023 and had to be
    # re-patched by hand after every refresh. Drives the Applications,
    # Acceptance & Yield chart + growth tables on the market page's
    ...
    "admissions_history": """
        SELECT
            ...
            s.name                                                  AS university_name,
            e.enrollmentYear                                        AS year_,
            e.appliedFirstTimeFirstYear                             AS applications,
            e.admittedFirstTimeFirstYear                            AS admitted,
            e.enrolledFirstTimeFirstYear                            AS enrolled
        FROM dbo.Schools s
        JOIN dbo.Markets m ON m.[Key] = s.MarketKey
        JOIN dbo.Enrollments e ON e.schoolKey = s.SchoolKey
        LEFT JOIN dbo.IPEDS_CH_Crosswalk cx
               ON cx.[Key] = s.SchoolKey AND cx.marketKey = s.MarketKey
        WHERE e.appliedFirstTimeFirstYear > 0
        ORDER BY s.SchoolKey, e.enrollmentYear
    """,
    "fte_history": """
    ...
    # Identity / coordinates from dbo.Schools; enrollment value/year from the
    # dbo.Enrollments view at the latest year with real enrollment (one row
    # per school - the old Schools_Denormal query emitted one duplicate row
    # per year and the UI deduped). Schools_Denormal flagged do-not-use
    # 2026-07-14.
    "campus_locations": """
        WITH latest_enr AS (
            SELECT e.*,
        ...
        SELECT
            ...
            cx.IPEDs                                                AS ipeds_id,
            s.name                                                  AS university_name,
            s.latitude                                              AS campus_lat,
            s.longitude                                             AS campus_lng,
            e.enrollmentTotal                                       AS total_enrollment,
            e.enrollmentYear                                        AS enrollment_year
        FROM dbo.Schools s
        JOIN dbo.Markets m ON m.[Key] = s.MarketKey
        LEFT JOIN latest_enr e
               ON e.schoolKey = s.SchoolKey AND e.rn = 1
        LEFT JOIN dbo.IPEDS_CH_Crosswalk cx
               ON cx.[Key]     = s.SchoolKey
              AND cx.marketKey = s.MarketKey
    ...
    #
    # total_enrollment / enr_full_time come straight from MarketReports at the
    # latest snapshot. (They previously came from Enrollments_Manual, which
    # was flagged do-not-use on 2026-07-14.)
    "scorecard": """
        WITH latest AS (
            SELECT market_key, MAX(snapshot_date) AS max_snap
            FROM dbo.MarketReports
        ...
        anchor AS (
            ...
            OUTER APPLY (
                ...
                FROM dbo.Schools s
                JOIN dbo.Enrollments e ON e.schoolKey = s.SchoolKey
                WHERE s.MarketKey = m.[Key] AND e.enrollmentTotal > 0
                ORDER BY e.enrollmentTotal DESC, s.name
            ) a
        ),
        s30 AS (
            ...
            SELECT DISTINCT sch.MarketKey
            FROM dbo.Subtext30 s
            JOIN dbo.Schools sch ON sch.name = s.University
            WHERE sch.MarketKey IS NOT NULL
              AND NOT EXISTS (
